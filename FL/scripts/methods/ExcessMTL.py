...
class ExcessMTL(object):
    ...
    def train_one_round(self):
        ...
        self.dynamic_lambdas_update(updated_models, self.config['learning_rate_lambda'])
    ...
    def dynamic_lambdas_update(self, local_models, lr):
        ...
        w = w / self.initial_w
        if 'w' in self.config:
            self.dynamic_lambdas = [lmb_i * np.exp(lr * weight_i * w_i) for lmb_i, weight_i, w_i in zip(self.dynamic_lambdas, self.config['w'], w)]
        else:
            self.dynamic_lambdas = [lmb_i * np.exp(lr * w_i) for lmb_i, w_i in zip(self.dynamic_lambdas, w)]
