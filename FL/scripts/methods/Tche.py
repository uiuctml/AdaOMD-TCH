...
class Tche(object):
    ...
    def global_param_update(self, global_model, local_models, client_losses, gamma):
        m = self.config['m']
        if 'w' in self.config:
            weighted_client_losses = [w_i * f_i for w_i, f_i in zip(self.config['w'], client_losses)]
            max_id = weighted_client_losses.index(max(weighted_client_losses))
        else:
            max_id = client_losses.index(max(client_losses))

        weights = {}
        for name, param in local_models[max_id].named_parameters():
