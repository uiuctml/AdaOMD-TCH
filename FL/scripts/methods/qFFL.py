...
class qFFL(object):
    ...
    def global_param_update(self, global_model, local_models, client_losses, lr):
        q = self.config['q']
        client_losses = torch.tensor(client_losses)
        deltas_coeff = [torch.pow(li, q) for li in client_losses]
        local_grads = [self.get_grad_tensor(local_model) for local_model in local_models]
        hs = [q * torch.pow(li, q-1) * torch.pow(torch.linalg.norm(gi), 2) + 1.0 / lr * torch.pow(li, q)
              for li, gi in zip(client_losses, local_grads)]
        aggregation_weights = torch.tensor(deltas_coeff) / sum(hs)
        ...
        for name, param in global_model.named_parameters():
            if param.requires_grad:
                param.data -= grads[name]


    def get_grad_tensor(self, model):
        grad = torch.cat([param.grad.view(-1, ) for param in model.parameters() if param.grad is not None]).to('cpu')
        return grad
