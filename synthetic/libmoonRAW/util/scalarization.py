...
def omdgd_tche(f_arr, w, dynamic_lambdas, eta=1.):
    def project(p):
        ...
        return res
    agg_loss = torch.sum(dynamic_lambdas * w * f_arr, axis=1)
    dynamic_lambdas = dynamic_lambdas + eta * w * f_arr.detach() # GD
    for i in range(dynamic_lambdas.size(0)):
        dynamic_lambdas[i] = project(dynamic_lambdas[i])
    return agg_loss, dynamic_lambdas


def omdeg_tche(f_arr, w, dynamic_lambdas, eta=1.0):
    agg_loss = torch.sum(dynamic_lambdas * w * f_arr, axis=1)
    dynamic_lambdas = dynamic_lambdas * torch.exp(eta * w * f_arr.detach())
    dynamic_lambdas /= torch.sum(dynamic_lambdas, axis=1).reshape(-1,1)
    # assert torch.sum(dynamic_lambdas[1]) == 1., f'{torch.sum(dynamic_lambdas[1])}Wrong normalization!'
    return agg_loss, dynamic_lambdas
