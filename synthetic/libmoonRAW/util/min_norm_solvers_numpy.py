...
class MinNormSolver:
    ...
    def find_min_norm_element_PGD(grads):
        ...
        while iter_count < MinNormSolver.MAX_ITER:
            ...
            new_lam = MinNormSolver._projection2simplex(new_lam)
    ...
    def find_min_norm_element_PGD_H(grads, A, Bh, H):
        """
        We directly run the PGD until convergence
        ...
        gamma = 0.1
        while iter_count < MinNormSolver.MAX_ITER:
            new_lam_f = lam_f - gamma * np.dot(gradsA, np.dot(grads.T, lam)) 
            new_lam_f = MinNormSolver._projection2simplex(new_lam_f)
            
            new_lam_h = lam_h - gamma * (np.dot(gradsBh, np.dot(grads.T, lam)) - 1*H)
            
            new_lam = np.dot(A.T, new_lam_f) + np.dot(Bh.T, new_lam_h)

            change = np.sum(np.abs(new_lam - lam))
            if change < MinNormSolver.STOP_CRIT:
                break
            else:
                lam_f = new_lam_f
                lam_h = new_lam_h
    ...
    def find_min_norm_element_PGD_H_single(grads, grad_lamt, 
        ...
        while iter_count < iter_K:
            ...
            new_lam_h = lam_h - gamma*(np.dot(gradsBh, grad_lamt) - 0.1 * H)            
            new_lam = np.dot(A.T, new_lam_f) + np.dot(Bh.T, new_lam_h)
