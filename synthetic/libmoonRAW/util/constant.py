import numpy as np
from .scalarization import ls, mtche, tche, pbi, cosmos, invagg, soft_tche, soft_mtche, aasf, pnorm, omdgd_tche, omdeg_tche
from libmoonRAW.util.problems import get_problem
import os
...
def get_agg_func(agg, scaler, cosmos_hp=8.0):
    if agg == 'ls':
        return ls
    elif agg == 'aasf':
        return aasf
        ...
        softtche_func = lambda f_arr, w: soft_tche(f_arr, w, mu=scaler)
        return softtche_func
    elif agg == 'softmtche':
        return soft_mtche
    elif 'omdgdtche' in agg:
        omdgd_func = lambda f_arr, w, dynamic_lambdas: omdgd_tche(f_arr, w, dynamic_lambdas, eta=scaler)
        return omdgd_func
    elif 'omdegtche' in agg:
        omdeg_func = lambda f_arr, w, dynamic_lambdas: omdeg_tche(f_arr, w, dynamic_lambdas, eta=scaler)
        return omdeg_func
    else:
...
beautiful_dict = {
    ...
    'agg_mtche': 'Agg-mTche',
    'agg_omdgdtche': 'Agg-OMDGDTche',
    'agg_omdegtche': 'Agg-OMDEGTche',
    'agg_gomdgdtche': 'Agg-GreedyOMDGDTche',
    'agg_gomdegtche': 'Agg-GreedyOMDEGTche',
